...
export default function App() {
  ...
  const [activeSwatch, setActiveSwatch] = useState<{ name: string; image: string } | null>(null);
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900">
        ...
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-full left-0 pt-2 w-64 opacity-0 scale-95 pointer-events-none group-hover:opacity-100 group-hover:scale-100 group-hover:pointer-events-auto transition-all duration-200 z-50">
                <div className="bg-white border border-slate-100 shadow-xl rounded-2xl py-3">
                  {catalogData.map((cat) => (
                    <a
                      key={cat.id}
                  ...
                  {category.products.map((product, idx) => (
                    ...
                    >
                      ...
                      <div className="w-full lg:w-1/2 flex flex-col justify-center">
                        ...
                        {/* Muestrario de Colores / Modelos */}
                        {product.swatches && product.swatches.length > 0 ? (
                          <div className="mb-6">
                            <h4 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-3">Modelos y Colores Disponibles</h4>
                            <div className="grid grid-cols-4 sm:grid-cols-5 gap-2">
                              {product.swatches.map((swatch, sIdx) => (
                                <button
                                  key={sIdx}
                                  onClick={() => setActiveSwatch(swatch)}
                                  className="group flex flex-col items-center bg-slate-50 border border-slate-200 hover:border-brand-light p-1.5 rounded-xl transition-all hover:shadow-md focus:outline-none"
                                  title={`Click para ampliar ${swatch.name}`}
                                >
                                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden bg-slate-100">
                                    <img
                                      src={swatch.image}
                                      alt={swatch.name}
                                  ...
                                  <span className="text-[10px] font-bold text-slate-600 mt-1 truncate w-full text-center px-0.5">
                                    {swatch.name}
                                  </span>
                                </button>
                              ))}
      ...
      {activeSwatch && (
        ...
        >
          <div 
            className="bg-white rounded-3xl overflow-hidden shadow-2xl max-w-sm w-full border border-slate-100 p-6 flex flex-col items-center relative animate-in zoom-in-95 duration-200"
          ...
          >
            ...
            <h3 className="text-xl font-bold text-brand-dark mb-4 text-center">
              Diseño / Color: {activeSwatch.name}
            </h3>
            <div className="w-64 h-64 rounded-2xl overflow-hidden shadow-inner bg-slate-100 mb-4 border border-slate-100">
              <img 
            ...
            <p className="text-sm text-slate-500 text-center mb-4">
              Muestra extraída directamente del catálogo oficial.
            </p>
            <button
              onClick={() => setActiveSwatch(null)}
              className="bg-brand-dark text-white font-bold py-2.5 px-6 rounded-xl hover:bg-brand-light transition-all text-sm w-full"
            >
              Cerrar Vista
            </button>
          </div>
